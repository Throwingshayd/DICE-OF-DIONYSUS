...
class HouseRuleCard extends Card {
    ...
    applyRule(gameState, gameEngine = null) {
        ...
        switch (this.ruleType) {
            ...
            case 'dice':
                ...
                break;
            case 'libation':
                this.applyLibationEffect(gameState, gameEngine);
                break;
            default:
                console.warn(`Unknown rule type: ${this.ruleType}`);
                return false;
        }
    ...
    applyLibationEffect(gameState, gameEngine = null) {
        ...
        switch (this.id) {
            case 'kyphi_mead':
                // Enhance a die face to Parchment
                this.promptForDieFaceSelection(gameState, 'parchment', gameEngine);
                break;
                
            case 'tisane_hephaestus':
                // Enhance a die face to Steel
                this.promptForDieFaceSelection(gameState, 'steel', gameEngine);
                break;
                
            case 'ambrosial_krasi':
                // Enhance a die face to Gold
            ...
            case 'elixir_lethe':
                // Permanently reduce a die face by 1
                this.promptForDieFaceSelection(gameState, 'permanent_reduce');
                break;
            ...
            case 'distillate_masks':
                ...
                if (gameState.jokers && gameState.jokers.length > 0) {
                    const randomBoon = gameState.jokers[Math.floor(Math.random() * gameState.jokers.length)];
                    const enhancements = ['parchment', 'iron', 'steel', 'gold', 'mother_of_pearl', 'mirror', 'wild'];
            ...
            case 'the_eucharist':
                // Gain +1 worship level in god of choice
                const gods = Object.keys(gameState.worshipLevels || {});
                if (gods.length > 0) {
                    // For now, just increase a random god's worship level by 1
                    ...
                    const engine = gameEngine || window.game;
                    engine?.showMessage?.(`The Eucharist: ${randomGod} worship increased by 1!`);
                } else {
                    const engine = gameEngine || window.game;
            ...
            case 'divine_guidance':
                // Gain 2 random levels in any 2 scores
                const availableGods = Object.keys(gameState.worshipLevels || {});
                if (availableGods.length >= 2) {
                    // Shuffle gods and take first 2
                    const shuffledGods = [...availableGods].sort(() => Math.random() - 0.5);
                    ...
                    // If only one god available, increase it by 2
    ...
    // Prompt user to select a die face for enhancement
    promptForDieFaceSelection(gameState, enhancementType, gameEngine = null) {
        ...
        const engine = gameEngine || window.game;
        engine?.showMessage?.(`${this.name}: Select a die to apply ${enhancementName} enhancement!`);
    ...
    applyEnhancementToDie(gameState, dieIndex, enhancementType, targetFaceValue, gameEngine = null) {
        ...
        switch (enhancementType) {
            ...
            case 'iron':
                ...
                break;
            case 'steel':
                if (die.addFaceEnhancement(targetFace, 'steel')) {
            
                    message = `Die ${dieNumber} face ${targetFace} enhanced with Steel (Always adds +1 to its value).`;
                } else {
                    message = 'Failed to apply steel enhancement!';
                }
                break;
            case 'gold':
            ...
            case 'mother_of_pearl':
                if (die.addFaceEnhancement(targetFace, 'mother_of_pearl')) {
                    message = `Die ${dieNumber} face ${targetFace} enhanced with Mother of Pearl (adds adjacent pips).`;
                } else {
            ...
            case 'wild':
                if (die.addFaceEnhancement(targetFace, 'wild')) {
                    message = `Die ${dieNumber} face ${targetFace} enhanced with Wild (face-specific).`;
                } else {
