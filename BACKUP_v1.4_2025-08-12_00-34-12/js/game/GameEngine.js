...
class GameEngine {
    ...
    bindDOMElements() {
        this.dom = {
            ...
            sellModeButton: document.getElementById('sellModeButton'),
    ...
    setupDOMEventListeners() {
        ...
        if (this.dom.sellModeButton) {
            this.dom.sellModeButton.addEventListener('click', () => this.toggleSellMode());
        }
    ...
    confirmScore() {
        ...
        // Gain gold for scoring
        this.state.gold += 1;
        this.showMessage("+1 Gold for scoring!");
    ...
    calculateScore(category) {
        ...
        this.state.dice.forEach((die, index) => {
            ...
            console.log(`Die ${index + 1}: face=${die.getEffectiveFace()}, enhancements=`, die.faces);
    ...
    finishAnteAndOpenShop() {
        ...
        // Scale threshold for next ante. Keep it simple, linear growth.
        this.state.scoreThreshold += 250;
        ...
        if (eventType === 'score') {
            ...
            if (hasSacrificialDagger || (hasRitualKnife && this.state.lowerSanctumStreak >= 2)) {
                if (this.state.consumables.length < this.state.consumableSlots) {
                    const libation = new HouseRuleCard(CardData.houseRules[Math.floor(this.prng.random() * CardData.houseRules.length)]);
    ...
    toggleSellMode() {
        if (window.shopManager) {
            window.shopManager.toggleSellMode(this.state, this);
        }
    }
